...
/** Gateway detail — ES|QL scoped to regional ingest path (errors when degraded/offline) */
export function buildTelematicsGatewayDiscoverEsql(gateway) {
  if (!gateway) return buildTelematicsPipelineDiscoverEsql();

  const service = GATEWAY_REGION_SERVICE[gateway.region] || 'checkout';
  const filters = [`service.name == "${service}"`];
  if (gateway.status === 'degraded' || gateway.status === 'offline') {
    filters.push('log.level IN ("ERROR", "Error", "WARN", "Warning")');
  }
  const subsystem = IOT_SUBSYSTEM_BY_SERVICE[service] || 'Vehicle Gateway (OBD/CAN)';
