...
/** IoT / telematics subsystem labels (otel-demo services remapped for connected vehicles) */
export const TELEMATICS_IOT_EVAL = [
...
const GATEWAY_REGION_SERVICE = {
  Americas: 'checkout',
  EMEA: 'payment',
  APAC: 'cart',
};

/** otel-demo services use mixed log.level casing (error, Error, warn, Warning, …) */
const GATEWAY_REGION_SERVICES = {
  Americas: ['checkout', 'checkoutservice'],
  EMEA: ['payment', 'paymentservice'],
  APAC: ['cart'],
};

const TELEMATICS_FAULT_LEVEL_FILTER =
  'log.level IN ("ERROR", "Error", "error", "WARN", "Warning", "warn", "WARNING")';

const IOT_SUBSYSTEM_BY_SERVICE = {
  checkout: 'Vehicle Gateway (OBD/CAN)',
  payment: 'Subscription & Billing',
  cart: 'Fleet Provisioning',
};

/** Telematics tab — fleet-wide pipeline stats by IoT subsystem */
export function buildTelematicsPipelineDiscoverEsql() {
  return [
    `FROM ${TELCO_OTEL_INDEX}`,
    `| WHERE ${TELCO_OTEL_SERVICE_FILTER}`,
    TELEMATICS_IOT_EVAL,
    '| STATS volume = COUNT(*), errors = COUNT(*) WHERE log.level IN ("ERROR", "Error", "WARN", "Warning") BY iot_subsystem',
    '| EVAL error_rate_pct = ROUND(errors * 100.0 / volume, 3)',
    '| SORT volume DESC',
    '| LIMIT 10',
  ].join(' ');
...
/** Gateway detail — ES|QL scoped to regional ingest path (fault logs when degraded/offline) */
export function buildTelematicsGatewayDiscoverEsql(gateway) {
  if (!gateway) return buildTelematicsPipelineDiscoverEsql();

  const services = GATEWAY_REGION_SERVICES[gateway.region] || GATEWAY_REGION_SERVICES.Americas;
  const primaryService = GATEWAY_REGION_SERVICE[gateway.region] || 'checkout';
  const filters = [`service.name IN (${services.map(s => `"${s}"`).join(', ')})`];
  if (gateway.status === 'degraded' || gateway.status === 'offline') {
    filters.push(TELEMATICS_FAULT_LEVEL_FILTER);
  }
  const subsystem = IOT_SUBSYSTEM_BY_SERVICE[primaryService] || 'Vehicle Gateway (OBD/CAN)';

  return [
    `FROM ${TELCO_OTEL_INDEX}`,
    `| WHERE ${filters.join(' AND ')}`,
    `| EVAL iot_subsystem = "${subsystem}"`,
    '| KEEP @timestamp, iot_subsystem, log.level, body.text, host.name, trace.id',
    '| SORT @timestamp DESC',
    '| LIMIT 50',
  ].join(' ');
}
